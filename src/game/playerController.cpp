...
void PlayerController::OnUpdate(){
	glm::vec3 camVel;
	if (Input->GetKey(SDL_SCANCODE_W)) camVel -= glm::vec3(camera->GetTransform()->forward.x, 0.0f, camera->GetTransform()->forward.z)  * 0.1f;
	if (Input->GetKey(SDL_SCANCODE_S)) camVel += glm::vec3(camera->GetTransform()->forward.x, 0.0f, camera->GetTransform()->forward.z) * 0.1f;
	if (Input->GetKey(SDL_SCANCODE_A)) camVel -= glm::vec3(camera->GetTransform()->right.x, 0.0f, camera->GetTransform()->right.z) * 0.1f;
	if (Input->GetKey(SDL_SCANCODE_D)) camVel += glm::vec3(camera->GetTransform()->right.x, 0.0f, camera->GetTransform()->right.z)* 0.1f;
